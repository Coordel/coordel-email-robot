...
exports.handleChange = function(change, fn){
  ...
  Common.getUserApps(users, function(err, apps){
    
    
    //so we need to track the user apps so we can test if they want to get messages
    var userMap = {};
    ...
    for (var key in map){
      //console.log("key", key);
      var suppress = userMap[key].suppressEmail;
      //if this user didn't do the update, then alert when history exists
      if (change.doc.docType !== "message" && change.doc.username !== "UNASSIGNED" && change.doc.updater !== key && change.doc.history && change.doc.history.length > 0){
    ...
    Common.getUser(toApp.email, function(err, user){
      ...
      getTemplateHtml({from: fromApp, to: toApp, change: change, alert: alert}, function(err, tempHtml){
        ...
        if (alert){
          ...
          subject = nls.message.toUpperCase() + ': ' + change.target.name;
        }
...
function getTemplateHtml(args, fn){
  ...
  getMainHtml({change: change, alert: alert}, function(err, mainHtml){
    ...
    var sender = from.firstName + ' ' + from.lastName;
    var tempData = {
      sender: sender,
      mainMessage: mainHtml,
      button: getButtonHtml(from, to, change, alert),
      features: getFeaturesHtml(),
      unsubscribe: getUnsubscribeHtml(to.id, sender)
    };
    
    //console.log("unsubscribe", tempData.unsubscribe);
...
function getMainHtml(args, fn){
  ...
  if (alert){
    ...
    //it's a message
    data.messageSender = change.actor.name;
    data.time = dateFormat(new Date(change.time), 'dddd, mmmm dS, yyyy, h:MM:ss TT');
    data.message = Common.lineBreak(change.body);
    var html = Common.render(Common.getHtml('message','html'), data);
    fn(null, html);
  }
}


function getUnsubscribeHtml(id, from){
  var map = Plates.Map();
  map.where('href').is('/').insert('newurl');
  map.class('unsubscribeSender').to('unsubscribeSender');
  var data = {newurl: 'http://'+settings.url + '/unsubscribe/'+ id, unsubscribeSender: from };
  //console.log("unsubscribe data", data);
  return Common.render(Common.getHtml('unsubscribe','html'), data, map);
}

function getFeaturesHtml(verb){
...
function getButtonHtml(from, to, change, alert){
  ...
  if (to.invited){
    ...
    buttonData.linkUrl = 'http://'+ settings.coordelUrl + '/invite/'+to.id;
    
    bnName = 'sign-up';
  } else {
    //buttonMap.where('id').is('link').use('link to login').as('href');
     buttonData.linkUrl = 'http://' + settings.coordelUrl + '/login';
     bnName = 'sign-on';
  }
